...
def create_master_data_export(user):
    queries = {
        ...
        if connection.vendor == "postgresql" else
        """SELECT "idChequeImportLine", "chequeImportLineCode", FORMAT("chequeImportLineDate", 'yyyy-MM-dd') chequeImportLineDate, "chequeImportLineStatus" FROM "tblChequeSanteImportLine";""",
        "ModuleConfiguration": """ SELECT "id", "module", "version", "config", TO_CHAR("is_disabled_until", 'yyyy-MM-dd') is_disabled_until, "is_exposed", "layer" FROM "core_ModuleConfiguration" WHERE "layer"='fe' AND "is_exposed" is true; """
        if connection.vendor == "postgresql" else
        """SELECT "id", "module", "version", "config", FORMAT("is_disabled_until", 'yyyy-MM-dd') is_disabled_until, "is_exposed", "layer" FROM "core_ModuleConfiguration" WHERE "layer"='fe' AND "is_exposed" = 'true' """,
    }
